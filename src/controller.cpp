...
void Controller::updateController(Uint8 *keys, controllers *tempController)
{
	if(m_isCPU) {
		updateComputer();
		return;
	...
	bool passIsPressed = false;
	
	if(tempController != NULL)
	{
		if(tempController->isCrossPressed && tempController->canCrossBePressed)
			{
				passIsPressed = true;
				fprintf(stderr, "\nPress Cross");
			}
			if(tempController->isCirclePressed && tempController->canCircleBePressed)
			{
				shootIsPressed = true;

			}
			if(tempController->isUpPressed || tempController->leftStickY < 64)
			{
				upIsPressed = true;
			}
			
			
			if(tempController->isDownPressed || tempController->leftStickY > 192){
				downIsPressed = true;
			}
			
			if(tempController->isLeftPressed || tempController->leftStickX < 64)
			{
				leftIsPressed = true;
			}
			
			if(tempController->isRightPressed || tempController-> leftStickX > 192)
			{
				rightIsPressed = true;
			}
	}
	...
	if(keys[m_up] || upIsPressed) 	y--;
	if(keys[m_down] || downIsPressed) y++;
	if(keys[m_left] || leftIsPressed) x--;
	if(keys[m_right] || rightIsPressed) x++;
	...
	if(m_canKick) {	
		if(keys[m_shoot] || shootIsPressed) {
	...
	} else if(!(keys[m_shoot] | keys[m_pass]) || !(shootIsPressed | passIsPressed)) {
		m_canKick = true;
	}	
}
