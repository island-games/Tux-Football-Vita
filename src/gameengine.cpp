...
SDL_ShowCursor(SDL_DISABLE);
	...
	if(m_screen == 0) {
		...
		m_scoreFont->write(m_screen, ("Starting Renderer"), 10, 10);
		SDL_Flip(m_screen);
		m_renderer = new Graphics(m_screen);

		m_scoreFont->write(m_screen, ("Loading Pitch"), 10, 40);
		SDL_Flip(m_screen);
		m_pitch = new Pitch(m_renderer);

		m_scoreFont->write(m_screen, ("Loading Ball"), 10, 70);
		SDL_Flip(m_screen);
		m_ball = new Ball(m_renderer, m_pitch);

		m_scoreFont->write(m_screen, ("Loading Team 1"), 10, 100);
		SDL_Flip(m_screen);
		m_homeTeam = new Team(this, "Blue Utd.", "BLU", "team1", "graphics/homeplayermarker.png", m_pitch, m_ball, true);

		m_scoreFont->write(m_screen, ("Loading Team 2"), 10, 130);
		SDL_Flip(m_screen);
...
void GameEngine::gameLoop()
{
	...
	{
		playerController = new controllers(0);
	}
	
	if(playerController2 == NULL)
	{
		playerController2 = new controllers(1);
	}
...
void GameEngine::iterateEngine()
{
	...
	playerController->GetCanPresses();
	playerController2->GetCanPresses();
}
...
void GameEngine::updateKeyboard()
{
	...
	m_homeController->updateController(keys, playerController);
	m_awayController->updateController(keys, playerController2);

	m_gameStates[m_currentState]->update(keys);

	if(keys[m_quitKey] || playerController->isStartPressed || playerController2->isStartPressed) {
